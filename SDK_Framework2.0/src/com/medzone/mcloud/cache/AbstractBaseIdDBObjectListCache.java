...
/**
 * @author Robert
 * @see 使用UID或者RID，去匹配一条数据是不可信的。因为服务端或者本地，不同数据类型是分表的，这意味着同一条UID，
 * 可能会对应不同类型的测量数据。通常情况下缓存实例也是按表来调用，但特殊的是：“血氧/血氧长期，胎心率/胎动”是复用同一张表的。
 * 所以应对上述两种情况时，可能会造成读取到脏数据。诸如：{@link #queryForMeasureRID(String, int)}，@
 * {@link #queryForMeasureUID(BaseIdDatabaseObject)} ,@
 * {@link #queryForMeasureUID(String)},{@link #deleteForMeasureUID(String)}
 * 都是存在安全隐患的方法。
 * @since 2.0
 */
